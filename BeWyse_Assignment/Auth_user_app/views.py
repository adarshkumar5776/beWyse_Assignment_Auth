...
def user_register(request):
    if request.method == "POST":
        ...
        if any(
            ...
            return JsonResponse(
                {"message": "Only 100 characters are allowed for a field"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    if "uid" in request.session:
        ...
        if request.method == "POST":
            ...
            existing_user = user_collection.find_one(
                {"username": new_username, "_id": {"$ne": object_id}}
            )
            if existing_user:
                return JsonResponse(
            ...
            existing_user_with_email = user_collection.find_one(
                {"email": new_email, "_id": {"$ne": object_id}}
            )
            ...
            auth.update_user(localId, email=new_email)
